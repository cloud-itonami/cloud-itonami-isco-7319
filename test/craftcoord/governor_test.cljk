...
(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-artisan! st {:artisan-id "artisan-1" :name "Kobo Yamada"})
    (store/register-workshop! st {:workshop-id "W-1" :name "Kobo Handicraft Workshop" :max-supply-cost 2000})
    st))
...
(deftest hard-on-scope-excluded-op-finalize-handicraft-item
  (testing "finalizing a craft-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-handicraft-item) :confidence 0.99) st)]
      (is (:hard? v))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to finalize the handicraft item is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we finalize the handicraft item now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
    ...
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:artisan-id "artisan-1" :op o :workshop-id "W-1"
                        :stake :low :task "routine handicraft task"
