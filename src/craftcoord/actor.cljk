(ns craftcoord.actor
  "CraftCoordActor — the ISCO-08 7319 handicraft workshop
  ...
  resume after human sign-off. ISCO-08 7319 (Handicraft Workers Not
  ...
  on cloud-itonami-isco-7211's foundrycoord.actor (closest domain
  ...
  The unconditional invariant: the Craft Coordination Advisor can
  never directly commit an operating record the CraftCoordGovernor
  ...
  makes quality/safety-clearance decisions itself."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [craftcoord.advisor :as advisor]
            [craftcoord.governor :as governor]
            [craftcoord.store :as store]))

(defn build-graph
  "Build a compiled CraftCoordActor graph. `store` implements
  `craftcoord.store/Store`. `advisor` implements
  `craftcoord.advisor/Advisor` (defaults to `mock-advisor`).
